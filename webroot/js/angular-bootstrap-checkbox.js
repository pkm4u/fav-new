...
angular.module("ui.checkbox", []).directive("checkbox", function() {
	return {
		...
		restrict: "E",
		replace: "true",
		template: "<button type=\"button\" ng-style=\"stylebtn\" class=\"btn-default\" ng-class=\"{'btn-xs': size==='default', 'btn-sm': size==='large', 'btn-lg': size==='largest', 'checked': checked===true}\">" +
		...
		compile: function compile(elem, attrs, transclude) {
			...
			return function(scope, elem, attrs, modelCtrl) {
				...
				elem.bind("click", function(val) {
					
					
					scope.$apply(function(modVal) {
